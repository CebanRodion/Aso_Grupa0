...
public class DepozitAndrei implements DepozitInterface{

    private int dimensionStorage;
    private Queue<Integer> depozit = new LinkedList<>();

    public DepozitAndrei(int dimension){
    ...
    public synchronized void put(int[] var1, String threadName) {
        ...
        System.out.println("\t" + threadName + " a adaugat 1 produs");
    ...
    public synchronized int[] get(String threadName) {
        ...
        if(Consumer.getElementsConsumed() == 48){
            ...
            System.out.println("\t" + threadName + " a consumat 1 produs");
        }
        else
        {
            if(depozit.size() == 1){
